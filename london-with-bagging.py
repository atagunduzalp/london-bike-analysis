...
# drop rows which includes NA
# plt.show(missingno.matrix(data, figsize = (30,10)))
# plt.show(missingno.heatmap(data))
data = data.dropna()
...
from sklearn.ensemble import RandomForestRegressor,BaggingRegressor,GradientBoostingRegressor,AdaBoostRegressor, SVR
...
########################## END OF THE BEST PARAMETERS SELECTION #####################
...
rf = RandomForestRegressor(n_estimators= 1800, max_features = 'auto', random_state=42, max_depth=None, min_samples_leaf=2, min_samples_split=2, bootstrap=True)
...
print("RandomForestRegressor -> r2 score: "+ str(r2_rf) + " rmsle: " + str(rmsle_rf))
"""
# BaggingRegressor Model Creation
rmsle_bg=[]
rmsle_dict_rf={}
r2_bg= []

# n_estimators=10, max_samples=2, max_features=1 = -0.017275068969781815
# n_estimators=30 = 0.9463857797550821
# n_estimators=1000 = 0.9472414701586035
bg = BaggingRegressor(n_estimators=30, base_estimator=CustomSVR())
bg.fit(X_train, y_train)
predicted_counts = bg.predict(X_test)
rmsle_bg.append(np.sqrt(mean_squared_log_error(predicted_counts, y_test)))
r2_bg.append(r2_score(y_test, predicted_counts))
print("BaggingRegressor -> r2 score: "+ str(r2_bg) + " rmsle: " + str(rmsle_bg))
